...
"[project]/src/components/common/clerk-session-sync.tsx [app-client] (ecmascript)", ((__turbopack_context__) => {
"use strict";
        ...
        "ClerkSessionSync.useEffect": ()=>{
            ...
            if (user) {
                ...
                const fullName = user.fullName || [
                    user.firstName,
                    user.lastName
                ].filter(Boolean).join(" ") || user.username || email.split("@")[0] || "Student";
                const initials = user.firstName && user.lastName ? `${user.firstName[0]}${user.lastName[0]}`.toUpperCase() : fullName.slice(0, 2).toUpperCase();
                // Check if user is an admin or instructor in existing mock registry
                const matchedMock = __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$auth$2f$mock$2d$users$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["MOCK_USERS"].find({
                    "ClerkSessionSync.useEffect.matchedMock": (u)=>u.email.toLowerCase() === email
                }["ClerkSessionSync.useEffect.matchedMock"]);
                const role = matchedMock?.role || "student";
                const session = {
                    email,
                    name: fullName,
                    initials,
...
_c = ClerkSessionSync;
var _c;
__turbopack_context__.k.register(_c, "ClerkSessionSync");
if (typeof globalThis.$RefreshHelpers$ === 'object' && globalThis.$RefreshHelpers !== null) {
    __turbopack_context__.k.registerExports(__turbopack_context__.m, globalThis.$RefreshHelpers$);
}
}),
...
//# sourceMappingURL=src_20ckexf._.js.map