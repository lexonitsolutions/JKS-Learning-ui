      ...
      <div className="flex-1 space-y-6 overflow-y-auto p-4 pt-3 sm:p-6 lg:p-8">
                    ...
                    {r.categories.map((cat) => (
                      <div key={cat.label}>
                        <div className="flex justify-between text-xs sm:text-sm text-slate-600 dark:text-slate-400">
                          <span>{cat.label}</span>
                          <span className="font-semibold text-slate-900 dark:text-white">{cat.score}</span>
                        </div>
        ...
        <Reveal variant="stagger" className="grid grid-cols-1 gap-4 sm:gap-6 md:grid-cols-2">
          <TiltCard>
            <div className={`${GLASS_CARD} p-4 sm:p-6`}>
              <h3 className="flex items-center gap-2 text-base font-bold text-slate-900 dark:text-white">
                <CheckCircle2 className="h-5 w-5 text-emerald-600 dark:text-emerald-400" /> Strengths
              </h3>
          ...
          <TiltCard>
            <div className={`${GLASS_CARD} p-4 sm:p-6`}>
              <h3 className="flex items-center gap-2 text-base font-bold text-slate-900 dark:text-white">
                <AlertTriangle className="h-5 w-5 text-amber-500 dark:text-amber-400" /> Weaknesses
              </h3>
              <ul className="mt-3 space-y-2 text-xs sm:text-sm text-slate-600 dark:text-slate-300">
                {r.weaknesses.map((w) => (
        ...
        <Reveal variant="fade-up">
          <div className={`${GLASS_CARD} p-4 sm:p-6`}>
            <h3 className="flex items-center gap-2 text-base font-bold text-slate-900 dark:text-white">
              <ListChecks className="h-5 w-5 text-[#2563EB] dark:text-blue-400" /> Recommended Topics
            </h3>
        ...
        <div>
          <h3 className="text-base font-bold text-slate-900 dark:text-white">Question-by-Question Breakdown</h3>
                ...
                <div className="flex items-start justify-between gap-4">
                  ...
                  <span
                    className={`shrink-0 text-lg font-extrabold ${
                      q.score >= 75 ? "text-emerald-600 dark:text-emerald-400" : q.score >= 60 ? "text-amber-500 dark:text-amber-400" : "text-rose-600 dark:text-rose-400"
                    }`}
                  >
                    {q.score}
                  </span>
