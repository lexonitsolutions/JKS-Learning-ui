...
export function DashboardSidebar({ role = "student" }: { role?: "student" | "admin" }) {
  ...
  return (
    <aside className="hidden w-[260px] shrink-0 flex-col p-4 md:flex">
      <div className="flex h-full flex-col rounded-[22px] border border-white/70 bg-white/85 shadow-[0_10px_35px_-5px_rgba(20,50,100,0.08),0_2px_6px_rgba(0,0,0,0.02)] backdrop-blur-xl transition-all duration-300">
        {/* Logo & Header */}
        <div className="flex h-20 items-center justify-between px-6">
          <Link href={rootHref} className="flex items-center gap-1 text-xl font-bold tracking-tight text-slate-900">
        ...
        {/* Navigation Items */}
        <nav className="flex-1 space-y-1 px-3.5 py-2 overflow-y-auto">
          {items.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`group flex items-center gap-3.5 rounded-xl px-4 py-2.5 text-[14px] font-medium transition-all duration-200 ${
                  active
                    ? "bg-[#EFF6FF] text-[#2563EB] font-semibold shadow-xs"
                    : "text-slate-600 hover:bg-slate-50/80 hover:text-slate-900"
                }`}
              >
                <item.icon
                  className={`h-4 w-4 shrink-0 transition-colors ${
                    active ? "text-[#2563EB] stroke-[2.2]" : "text-slate-400 group-hover:text-slate-600 stroke-[1.8]"
                  }`}
                />
                <span>{item.label}</span>
              </Link>
